...
class RTLModule(BasePortalModule):
    ...
    def get_files(self, run_id: str, metadata_path: str | None) -> list[FileTransfer]:
        ...
        if metadata_path and os.path.isfile(metadata_path):
            ...
            candidates = (
                glob.glob(os.path.join(self.source_dir, "*.xml")) +
                glob.glob(os.path.join(self.source_dir, "*.xlsx"))
            )
        ...
        if meta:
            transfers.append(FileTransfer(
                ean=None,
                file_name=os.path.basename(meta),
                file_type="metadata",
                source_path=meta,
                destination=self._remote_path(os.path.basename(meta)),
    ...
    def ship(self, run_id: str, transfers: list[FileTransfer], progress_cb: ProgressCallback) -> None:
        with sftp_connection(self.host, self.port, self.username, self.password) as sftp:
            if self.remote_dir and self.remote_dir != "/":
                sftp_makedirs(sftp, self.remote_dir)
            for t in transfers:
                try:
